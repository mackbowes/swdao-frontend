...
const BondingCard = () => {
    ...
    const updateRates = async () => {
        ...
        setTextSwdRemaining(
            formatNumber(utils.formatUnits(swdRemaining, 18)) + ' SWD'
        )
    ...
    useEffect(() => {
        ...
        const balanceTooLow = utils
            .parseUnits(amountDeposit, 18)
            .gt(balanceUserSwx)
        setAmountDepositTooHigh(balanceTooLow)
        setAmountDepositVerified(
            !(balanceTooLow || parseFloat(amountDeposit) <= 0)
        )
    ...
    const withdrawAsync = async () => {
        toast('Bond withdrawal initiated')
        ...
        if (result.code == 1) {
            ...
            if (success) {
                ...
                toast('Withdrawal failed, or took too long')
    ...
    const depositAsync = async () => {
        ...
        if ((await approveCheck(ethersPack)).lt(amountDepositNum)) {
            toast('Token approval initiated')
            ...
            if (result.code == 1) {
                ...
                for (let i = 0; i < 12 && !approved; i++) {
                    await new Promise((r) => setTimeout(r, 5000))
                    approved = (
                        await approveCheck(ethersPack)
                    ).gte(amountDepositNum)
                }
                if (approved) {
                    toast("Approval successful")
                } else {
                    console.error(
                        '(depositAsync): approveCheck failed.',
                        approved
                    )
                    toast('Approval failed, or took too long')
                    setTxInProgress(false)
                    return
                }
            } else {
        ...
        if (result.code == 1) {
            ...
            if (success) {
                ...
                toast('Deposit failed, or took too long')
                ...
                {isDepositModalOpen && (
                    <Box className={styles.modal}>
                        ...
                        >
                            ...
                            <Box className={styles.modalCurrencyIndicator}>
                                SWX
                            </Box>
                        </Box>
                        <DepositButton
                        ...
                        {bonds.length > 0 && (
                            <Box>
                                <Box
                                    className={styles.bondTableHeader}
                                    sx={{ color: `#AADCFE` }}
                                >
                                    <p>Deposit</p>
