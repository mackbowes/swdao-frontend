...
export const getSwxBalance = async (address) => {
    ...
    return await (await fetch('/bonds/api/getUserBalance', req)).json()
}
...
export const getContractInfo = async () => {
	...
	const slot0 = await (await fetch('/bonds/api/getStorageAt', req)).json()
    ...
    const mpy = (100 * Math.pow(apy / 100 + 1, 1 / 12) - 100)
        .toString()
        .substring(0, 4)
    const wpy = (100 * Math.pow(apy / 100 + 1, 1 / 52) - 100)
        .toString()
        .substring(0, 4)
...
export const getSwdAvailable = async (address) => {
	...
	return await (await fetch('/bonds/api/getUserSwdAvailable', req)).json()
}
...
export const getBondsPage = async (address, page, amountToView) => {
    ...
    return await (await fetch('/bonds/api/getBondsPage', req)).json()
}